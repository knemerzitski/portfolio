...
function BaseInput<C extends ElementType>({
  ...
  return (
    <div className="relative">
      <Component ref={handleRef} className={classNames(
        'block',
        'w-full',
        'p-3 rounded',
        'bg-transparent',
        'outline',
        'outline-1',
        isInvalid ? 'outline-2 outline-danger-500' :
          (isActivated ? 'outline-secondary-800 focus:outline-secondary-600'
      ...
      <div className={classNames(`bg-${surfaceColor}`, 'absolute text-sm text-text/80 select-none leading-none -top-[1ch] left-2 px-1')}>
        {label}
      </div>

      <div className={classNames('mt-1 text-sm', isInvalid && 'text-danger-500')}>
